...
/*   Updated: 2023/03/29 19:09:38 by jewancti         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static
int	get_color_from_texture(t_data *data, double *texture_pos, int texture_x)
{
	int	color;
	int	texture_y;
...
void	end_textures(t_data *data, int x, double text_pos, const int colors[3])
{
	...
	while (++y < HEIGHT)
	{
		if (y < data -> params.draw_start)
			...
			color = get_color_from_texture(data, & text_pos, texture_x);
		data -> buf[y][x] = color;
